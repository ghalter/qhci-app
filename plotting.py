...
def generate_data(slope_type=SLOPE_TYPE_QUAD, n=100, slope=1.0, amplitude = 1.2, exponent = 2.5, plot_type=GRAPH_TYPE_AREA):
    xs = []
    ys = []
    amplitude = np.random.uniform(1, 10, 1)
    a = 1.2
    q = 0.1
    f = 2.5
    ...
    if slope_type == "linear":
        for i in range(n):
            xs.append(i / n)
            ys.append((slope * (i / n)) + np.random.uniform(-.10, .10, 1))
        trend_line = ColumnDataSource(data=dict(x=[0.0, 1.0], y=[0, 1.0], slope=[0, 0]), name="slope_ds")
        ...
        for i in range(n):
            x = i / n
            e = np.random.uniform(-0.1, 0.1, 1) * np.sqrt(x)
            xs.append(i / n)
            ys.append(x ** f + e)
            trend_xs.append(x)
            trend_ys.append(x)
        trend_line = ColumnDataSource(data=dict(x=trend_xs, y=trend_ys), name="slope_ds")
        ...
        for i in range(n):
            xs.append(i / n)
            ys.append(np.sin(i * q) * amplitude + np.random.uniform(-1, 1))
            trend_xs.append(i / n)
            trend_ys.append(np.sin(i * q))
        trend_line = ColumnDataSource(data=dict(x=trend_xs, y=trend_ys), name="slope_ds")
...
def generate_plot(ds, trend_line, plot_type = GRAPH_TYPE_AREA, slope_type = SLOPE_TYPE_QUAD):
    ...
    if slope_type == SLOPE_TYPE_LIN:
        ...
        callback = CustomJS(args=dict(source=trend_line), code="""
                    ...
                    source.change.emit();
                """)
        amp_slider = Slider(start=0, end=20, value=0.0, step=.1,
                            title=None, callback=callback)
        ...
        callback = CustomJS(args=dict(source=trend_line), code="""
                    ...
                    source.change.emit();
                """)
        amp_slider = Slider(start=0, end=6, value=0.0, step=.1,
                            title=None, callback=callback)
        callback.args["slope"] = amp_slider
